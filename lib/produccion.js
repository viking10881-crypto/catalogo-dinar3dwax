...
/**
 * Envía un pedido aprobado al taller (dinar-3dwax) como uno o más OT — un
 * OT por cada línea del carrito (referencia + talla), no por unidad: si
 * el cliente pidió la misma pieza × 2, es un solo OT con cantidad=2 (un
 * lote de producción), para que en el tablero y en la cuenta de cobro se
 * vea como una sola tarjeta/línea del pedido. Usa un secreto compartido
 * (no hay login entre apps) y es idempotente del lado del taller (mismo
 * origenCatalogoId no duplica), así que reintentar aquí es seguro.
 */
...
async function construirPiezas(pedido) {
  ...
  for (let i = 0; i < pedido.items.length; i++) {
    ...
    piezas.push({
      origenCatalogoId: `${pedido.id}#${i}`,
      descripcionPieza: descripcion,
      cantidad,
      // pesoCera es el peso de UNA pieza (no se multiplica); precioTotal
      // sí es el valor del lote completo (precioUnitario × cantidad).
      pesoCera,
      metalNombre,
      precioTotal: (Number(item.precioUnitario) || 0) * cantidad,
      imagenUrl,
      archivoStlUrl,
      archivoStlNombre,
    });
